...
class SMTP extends MIME {
	...
	function _writemsg(){
		...
		foreach($this->_toaddrs as $taddr => $tname){
			$toarr[] = ($tname != "") ? '"'.str_replace('"', '\\"', $tname).'" <'.$taddr.'>' : $taddr;
		}
		$tostr = implode(', '.$this->_crlf."\t", $toarr);
		$hto = implode(', ', $toarr);
		if($this->_ccaddrs && count($this->_ccaddrs) > 0){
			foreach($this->_ccaddrs as $caddr => $cname){
				$ccarr[] = ($cname != "") ? '"'.str_replace('"', '\\"', $cname).'" <'.$caddr.'>' : $caddr;
			}
			$ccstr = implode(', '.$this->_crlf."\t", $ccarr);
		}
		if($this->_bccaddrs && count($this->_bccaddrs) > 0){
			foreach($this->_bccaddrs as $baddr => $bname) $bccarr[] = $baddr;
			$bccstr = implode(', '.$this->_crlf."\t", $bccarr);
		}
		...
		$xmail = array('name' => base64_decode('WC1NYWlsZXI='), 'value' => base64_decode('WFBNMiB2LjAuMyA8d3d3LnhwZXJ0bWFpbGVyLmNvbT4='));
		$arrval1[] = $xmail;
		$arrval2[] = $xmail;
		$hval1 = $hval2 = $bval = '';
