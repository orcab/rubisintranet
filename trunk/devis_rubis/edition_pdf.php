...
$sql_entete = <<<EOT
select NOBON,DSECM,DSECJ,DSECS,DSECA,LIVSB,RFCSB,BUDSB,AD1SB,AD2SB,CPOSB,NOMSB,VALFS,VALFA,VALFM,VALFJ
from ${LOGINOR_PREFIX_BASE}GESTCOM.AENTBVP1
where	NOBON='$NOBON_escape'
	and NOCLI='$NOCLI_escape'
EOT;
...
while($row = odbc_fetch_array($detail_devis)) {
	...
	if ($row['CONSA'] && $row['PRINE'] <= 0) { // cas d'un commentaire
		...
		$total_ht		+= $row['QTESA']*$row['PRINE']; // on rajoute la somme au total
