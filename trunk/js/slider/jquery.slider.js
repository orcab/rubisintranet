...
jQuery.fn.makeSlider = function(scaleInterval, sliderWidth, selectsVisible){
	$(this).each(function(){
		...
		$(indicators).each(function(l){
			...
			sliderComponent.find('.sliderControl').append('<div class="indicator'+thisInt+' sliderIndicator" role="slider" aria-valuemin="'+ minVal +'" aria-valuemax="'+  maxVal+'"><span class="indicatorFeedback'+thisInt+'"></span></div>');
		});
		if(optGroups){
		...
		sliderComponent.Slider({
			...
			onSlide : function( cordx, cordy, x , y) {
				...
				//indicator and aria
				feedback.html(allValues[uniqueID][convertX]).parent().attr('aria-valuetext', allValues[uniqueID][convertX]).attr('aria-valuenow', allValues[uniqueID][convertX]);
