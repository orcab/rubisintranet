...
if (!(isset($_GET['HIBON']) && $_GET['HIBON'])) { ?>
	ERREUR : Aucun N° de cde précisé.
<? 	exit;
}

$cfbon_escape = mysql_escape_string($_GET['HIBON']);
...
from ${LOGINOR_PREFIX_BASE}GESTCOM.ACFENTP1, ${LOGINOR_PREFIX_BASE}GESTCOM.AFOURNP1
where	CFBON='$cfbon_escape'
	and ${LOGINOR_PREFIX_BASE}GESTCOM.ACFENTP1.NOFOU=${LOGINOR_PREFIX_BASE}GESTCOM.AFOURNP1.NOFOU
EOT;

$sql_detail = <<<EOT
select CFART,CFCLB,CFDDA,CFDDS,CFDDM,CFDDJ,CFDLA,CFDLS,CFDLM,CFDLJ,REFFO,CFDE1,CFDE2,CFDE3,CFUNI,CFQTE,CFPAB,CFRE1,CFRE2,CFRE3,CFTY1,CFTY2,CFTY3,CFPAN,CFMTH,CFDTY,CFCLI,CFPRF,CFCOM
...
while($row = odbc_fetch_array($detail_commande)) {
	...
	if ($row['CFPRF'] == 9) { // cas d'un commentaire
		...
		$pdf->Row(	array( //   font-family , font-weight, font-size, font-color, text-align
					array('text' => $row['REFFO']?$row['REFFO']:"$row[CFART]\n(code ARB 114)", 'font-style' => 'B',	'text-align' => 'C', 'font-size' => strlen($row['REFFO'])>10 ? 8:10 ),
		...
		//$total_ht		+= $row['CFMTH']; // on rajoute la somme au total
		//$sous_total_ht	+= $row['QTESA']*$row['PRINE']; // on rajoute la somme au sous total
	}
}
