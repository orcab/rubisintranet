...
define('CODAR_WIDTH',20);
define('FOURNISSEUR_WIDTH',30);
...
define('DESIGNATION_WIDTH',PAGE_WIDTH - LEFT_MARGIN - RIGHT_MARGIN - (CODAR_WIDTH + FOURNISSEUR_WIDTH + UNITE_WIDTH + QTE_WIDTH + PU_WIDTH + TOTAL_WIDTH + TYPE_CDE_WIDTH) ); // s'appadate à la largeur de la page

class PDF extends FPDF
{
	//EN-TÊTE
	function Header() {
		global $row_entete,$SOCIETE,$row,$arguments ;
		
		$this->Image('gfx/filigranne_livraison.png',0 ,0, PAGE_WIDTH, PAGE_HEIGHT); // filigranne en fond de page
		...
		// qrcode du fichier
		/*
		$json = array('t'=>'bon_livraison','b'=>$row_entete['NOBON'],'c'=>$row_entete['NOCLI'],'d'=>time(),'p'=>$this->PageNo(),'u'=>$arguments['user']);
		$qrcode = new QRcode(json_encode($json), 'L'); // error level : L, M, Q, H
		$qrcode->displayFPDF($this, PAGE_WIDTH - 72 , 0, 20);
		*/
		...
		$type_livraison = '';
		if		($row_entete['TYVTE']=='LIV')
			$type_livraison = 'LIVRÉ';
		else if ($row_entete['TYVTE']=='EMP')
			$type_livraison = 'EMPORTÉ';

		// titre
		$this->SetFont('helvetica','B',12);
		$this->SetTextColor(255,0,0);
		$this->Cell(0,5,"BON DE LIVRAISON $type_livraison".($arguments['duplicata'] ? ' (Duplicata)':''),0,1,'C');
		$this->Ln(0.5);
		...
		$this->Cell(CODAR_WIDTH,8,"Code",1,0,'C',1);
		$this->Cell(FOURNISSEUR_WIDTH,8,"Fournisseur",1,0,'C',1);
		$this->Cell(DESIGNATION_WIDTH,8,"Désignation",1,0,'C',1);
		$this->Cell(UNITE_WIDTH,8,"Unit.",1,0,'C',1);
		$this->Cell(QTE_WIDTH,8,"Qté",1,0,'C',1);
		$this->Cell(PU_WIDTH,8,"PU",1,0,'C',1);
		$this->Cell(TOTAL_WIDTH,8,"Tot",1,0,'C',1);
	...
	function Footer() {
		...
		$this->Cell(0,5,"Toute réclamation doit être faite dans les 48 heures après la livraison. La date d'édition du BL faisant foi.",0,1,'C');
