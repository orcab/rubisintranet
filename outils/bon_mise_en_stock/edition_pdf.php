...
$sql_detail = <<<EOT
select CFLIG,CFART,CFCLB,CFDDA,CFDDS,CFDDM,CFDDJ,CFDLA,CFDLS,CFDLM,CFDLJ,A_F.REFFO,CFDE1,CFDE2,CFDE3,CFUNI,CFQTE,CFCLI,CFPRF,CFCOM,LOCAL,LOCA2,LOCA3,CFPAN,CFMTH,
...
from ${LOGINOR_PREFIX_BASE}GESTCOM.ACFDETP1 DETAIL
	...
	left join ${LOGINOR_PREFIX_BASE}GESTCOM.AARFOUP1 A_F
		on		ARTICLE.NOART=A_F.NOART
			and ARTICLE.FOUR1=A_F.NOFOU
where	CFBON	=	'$cfbon_escape'
	and CFDET	=	''
...
while($row = odbc_fetch_array($detail_commande)) {
	...
	if ($row['CFPRF'] == 9) { // cas d'un commentaire
		if ($row['CFCOM']) {
			if (preg_match('/^ +/',$row_original['CFCOM']))// un espace devant le commentaire défini un COMMENTAIRE
		...
		if (preg_match('/^\d{13}$/',$row['GENCO']) && $row['KIT'] != 'OUI' && is_ean13($row['GENCO'])) {
...
EOT;
			...
			while($row_kit = odbc_fetch_array($res_kit)) { // on parcours les articles du kit et on les enregistre pour plus tard
				...
				// affichage du code barre du produit si c'est pas un kit
				if ($row_kit['GENCO'] && is_ean13($row_kit['GENCO'])) {
					$pdf->SetFillColor(0,0,0); // noir
