...
require_once('overload_test.php');
...
if (!(isset($arguments['nobon']) && $arguments['nobon'])) {
	echo "ERREUR : parametre --nobon vide ou manquant"; exit;
}
if (!(isset($arguments['nocli']) && $arguments['nocli'])) {
	echo "ERREUR : parametre --nocli vide ou manquant"; exit;
}
...
$sql_entete = <<<EOT
select	NOBON,DSECS,DSECA,DSECM,DSECJ,NOMSB,AD1SB,AD2SB,CPOSB,BUDSB,DLSSB,DLASB,DLMSB,DLJSB,RFCSB,MONTBT,TELCL,TLCCL,TOUCL,TELCC,TLXCL,COMC1,
...
if (isset($arguments['lignes']) && $arguments['lignes']) {
	...
	array_push($tmp,"BON.PROFI='1' or PROFI='9'");	// ajout des commentaires
...
if (DEBUG) {
	echo "SQL_ENTETE : $sql_entete";
	echo "SQL_DETAIL : $sql_detail";
	exit;
}
...
if (!is_array($row_entete)) {
	echo "ERREUR : Aucun bon de retrouve avec les parametres --nobon=$arguments[nobon] --nocli=$arguments[nocli]"; exit;
}
...
$filename = 'bon_prepa_'.$arguments['nobon'].'('.crc32(uniqid()).').pdf';	# defnit un nom de fchier unique
$pdf->Output($filename,'F');												# creer le fichier PDF
echo print_time()."File '$filename' generated\n";
...
if (isset($arguments['printer']) && $arguments['printer'] &&
	...
	echo print_time()."Sending to printer '$arguments[printer]'... ";
	system('"'.$arguments['foxit_path'].'" -t '.$filename.' '.$arguments['printer']);# envoie le fichier PDF vers l'imprimante
	if (BON_DE_RETOUR) // deux exemplaire pour un bon de retour
		system('"'.$arguments['foxit_path'].'" -t '.$filename.' '.$arguments['printer']);# envoie le fichier PDF vers l'imprimante
	echo "ok\n";

	echo print_time()."Deleting PDF file... ";
