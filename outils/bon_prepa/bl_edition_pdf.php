...
while($row = odbc_fetch_array($detail_commande)) {
	...
	if (isset($row['CODAR']) && isset($row['QTESA']) && isset($row['TRAIT']) && $row['CODAR']) { // les commentaires n'ont pas de CODAR
		if ($lignes_reste_imprimer[$row['CODAR'].':'.$row['QTESA'].':'.$row['TRAIT']] > 0) // cette ligne reste a imprimer
			$lignes_reste_imprimer[$row['CODAR'].':'.$row['QTESA'].':'.$row['TRAIT']]-- ; // on descent le nombre de ligne a imprimer (bug des lignes en double)
