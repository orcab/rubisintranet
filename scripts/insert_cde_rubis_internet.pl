...
my $thirty_days_ago			= strftime('%Y-%m-%d'	,0,0,0,	strftime('%d', localtime) - 31, strftime('%m', localtime) - 1, strftime('%Y', localtime) - 1900) ;
my $thirty_days_ago_rubis	= strftime('%Y%m%d'		,0,0,0,	strftime('%d', localtime) - 31, strftime('%m', localtime) - 1, strftime('%Y', localtime) - 1900) ;
...
if (!$skip{'delete'}) {
	...
	if (exists $options{'from'} && exists $options{'to'}) {
		...
		$sqlite->do("DELETE FROM cde_rubis WHERE date_bon >= '$thirty_days_ago'") ;
	}
	print "OK\n";
}
...
if (!exists $options{'all'}) { # on n'indexe que le dernier mois
	if (exists $options{'from'} && exists $options{'to'}) {
		...
		$where_date_bon = " and CONCAT(DTBOS,CONCAT(DTBOA,CONCAT(DTBOM,DTBOJ))) >= '$thirty_days_ago_rubis' " ;
	}
}
...
while($loginor->FetchRow()) {
	...
	if ($old_bon ne "$row{NOBON}.$row{NOCLI}") { #nouveau
		$sqlite->do("INSERT OR IGNORE INTO cde_rubis (id_bon,numero_bon,numero_artisan,date_bon,date_liv,vendeur,nb_ligne,montant,reference,agence) VALUES ('$row{NOBON}.$row{NOCLI}','$row{NOBON}','$row{NOCLI}','$row{DATE_BON}','$row{DATE_LIV}','$row{LIVSB}',$row{NBLIG},$row{MONTBT},'$row{RFCSB}','$row{AGELI}')");
		if ($sqlite->err()) { die "$DBI::errstr\n"; }
	}

	#insertion des différente ligne du bon
	$sqlite->do("INSERT OR IGNORE INTO cde_rubis_detail (id_bon,code_article,fournisseur,ref_fournisseur,designation,unit,qte,prix,spe) VALUES ('$row{NOBON}.$row{NOCLI}','$row{CODAR}','$row{NOMFO}','$row{REFFO}','$designation','$row{UNICD}',$row{QTESA},$row{PRINE},".($row{'TYCDD'} eq 'SPE' ? 1:0).")");
	if ($sqlite->err()) { die "$DBI::errstr\n"; }
...
if (!$skip{'delete'}) {
	...
	if (exists $options{'from'} && exists $options{'to'}) {
		...
		$sqlite->do("DELETE FROM devis_rubis WHERE date_bon >= '$thirty_days_ago'");
	}
	print "OK\n";
}
...
if (!exists $options{'all'}) { # on n'indexe que le dernier mois
	if (exists $options{'from'} && exists $options{'to'}) {
		...
		$where_date_devis = " and CONCAT(DTBOS,CONCAT(DTBOA,CONCAT(DTBOM,DTBOJ))) >= '$thirty_days_ago_rubis' " ;
	}
}
...
while($loginor->FetchRow()) {
	...
	if ($old_bon ne "$row{NOBON}.$row{NOCLI}") { #nouveau
		$sqlite->do("INSERT OR IGNORE INTO devis_rubis (id_bon,numero_bon,numero_artisan,date_bon,date_liv,vendeur,nb_ligne,montant,reference,agence) VALUES ('$row{NOBON}.$row{NOCLI}','$row{NOBON}','$row{NOCLI}','$row{DATE_BON}','$row{DATE_LIV}','$row{LIVSB}',$row{NBLIG},$row{MONTBT},'$row{RFCSB}','$row{AGELI}')");
		if ($sqlite->err()) { die "$DBI::errstr\n"; }
	}

	#insertion des différente ligne du bon
	$sqlite->do("INSERT OR IGNORE INTO devis_rubis_detail (id_bon,code_article,fournisseur,ref_fournisseur,designation,unit,qte,prix,spe) VALUES ('$row{NOBON}.$row{NOCLI}','$row{CODAR}','$row{NOMFO}','$row{REFFO}','$designation','$row{UNICD}',$row{QTESA},$row{PRINE},".($row{'TYCDD'} eq 'SPE' ? 1:0).")");
	if ($sqlite->err()) { die "$DBI::errstr\n"; }
...
while($loginor->FetchRow()) {
	...
	#insertion des différente ligne du bon
	$sqlite->do("INSERT OR IGNORE INTO reliquat_detail (id_bon,code_article,fournisseur,ref_fournisseur,designation,unit,qte,prix,spe,date_dispo) VALUES ('$row{NOBON}.$row{NOCLI}','$row{CODAR}','$row{NOMFO}','$row{REFFO}','$designation','$row{UNICD}',$row{QTESA},$row{PRINE},".($row{'TYCDD'} eq 'SPE' ? 1:0).",'$date_dispo')");
	if ($sqlite->err()) { die "$DBI::errstr\n"; }
...
#	$ftp->login('coopmcs','k73nk9vR49') or die "Cannot login ", $ftp->message;
