...
function liste_toute_relance() {
	...
	for(i=0 ; i<tr_elements.length ; i++) {
		if (tr_elements[i]['id'].match(/^relance_devis_\d+$/))
...
function calcul_cmd_rubis(id) {
	param = new Array('what=calcul_cmd_rubis');
	$('#cmd_rubis_'+id).html('<img src="gfx/loading.gif">');
	param.push('id[]='+id);
	
	http.open('GET','ajax.php?'+param.join('&'), true);
	http.onreadystatechange = function() {
		if (http.readyState == 4) {
			var result = eval('(' + http.responseText + ')') ;
			var ptht_cmd_rubis = 0 ;
			for(i=0 ; i<result.length ; i++ ) {
				$('#cmd_rubis_'+result[i][0]).html(result[i][1] + ' &euro;');
				ptht_cmd_rubis += result[i][1] ;
			}
			$('#ptht_cmd_rubis').html((Math.round(ptht_cmd_rubis * 100)/100) + ' &euro;');
		}
	};
