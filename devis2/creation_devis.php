...
function draw_page(pageno) {
	...
	for(i=nb_results_by_page * (pageno-1) ; i<all_results.length && i<nb_results_by_page * (pageno-1) + nb_results_by_page ; i++) {
		div.append(	'<tr onclick="insert_ligne(\''+all_results[i].id+'\');">' + 
									'<td>' + all_results[i].reference.replace(recherche,'<strong>'+recherche+'</strong>') + '<td>' +
									'<td>' + all_results[i].fournisseur + '<td>' +
									'<td>' + all_results[i].designation + '<td>' +
									'<td>' + Math.round(all_results[i].px_public * 100)/100 + '<td>' +
								'</tr>'
...
function recalcul_total() {
	...
	$('input[name=a_qte]').each(function() {
		...
		if (pu >= 0 && qte >=0) {
			...
			$(this).parents('tr').children('td[name=a_pt]').text(val + '€');
			total += val ;
		} else {
...
<fieldset class="total">
    <legend>Total :</legend>
	<div>Total <sup>ht</sup>&nbsp;:&nbsp;&nbsp; <span id="total"></span> €</div>
</fieldset>
